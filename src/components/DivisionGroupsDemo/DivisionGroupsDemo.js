          ...
          {range(numOfGroups).map((groupIndex) => (
            <div key={groupIndex} className={styles.group}>
              {range(numOfItemsPerGroup).map((index) => {
                ...
                let prevGroupNumber = groupIndex * numOfItemsPerGroup;
                let layoutId = `${id}-${prevGroupNumber + index}`;
      ...
      {includeRemainderArea && (
        <div className={styles.remainderArea}>
          ...
          {range(remainder).reverse().map((index) => {
            let totalInGroups = numOfGroups * numOfItemsPerGroup;
            let layoutId = `${id}-${totalInGroups + index}`;
            return (
